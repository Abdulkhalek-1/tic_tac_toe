    ...
    if (typeof param === "string" || param instanceof String) {
        if (param.includes("<")) {
            return new ElementCollection(
                ...new DOMParser().parseFromString(param, "text/html").body
                    .childNodes
            );
        } else {
...
function setAnimation(id, value) {
    ...
    const symbols = {
        x: `<svg xmlns="http://www.w3.org/2000/svg" class="h-1/2 w-1/2 text-white pointer-events-none select-none duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 20 20 4M4 4 20 20" /></svg>`,
        o: `<svg xmlns="http://www.w3.org/2000/svg" class="h-1/2 w-1/2 text-white pointer-events-none select-none duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728" /></svg>`,
        "": "",
    };
    if (value) {
...
function copy() {
    const gameIdElement = document.querySelector(".game_id");
    navigator.clipboard.writeText(window.location);

    if (!gameIdElement.classList.contains("copied")) {
        gameIdElement.classList.add("copied");
        setTimeout(() => {
            gameIdElement.classList.remove("copied");
        }, 3000);
    }
}
